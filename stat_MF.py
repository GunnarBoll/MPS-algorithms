...
def main():
    ...
    while i < 10:
        ...
        err = (abs(ord_pars[i]) - abs(new_ord_par)) / abs(ord_pars[i])
    ...
    popt, pcov = curve_fit(exp_func, range(1, 9), ord_pars[0 : 8],
                                p0=(-1, 1, 2))
    guess = 4 * tperp * popt[2]
    ex_res = extrap_res(g1, [mu_list[-1], guess], N, dt, d, chi, model, order,
                        algo, step_num, a)
    print(abs(ex_res), ord_pars[-1], ord_pars[6])
